...
class RegressionDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str) -> None:
        ...
        x_noise = np.random.normal(0.0, self.x_noise_std, size=x.shape)
        ...
        y = (
            x
            + 0.3 * jnp.sin(2 * jnp.pi * (x + x_noise))
            + 0.3 * jnp.sin(4 * jnp.pi * (x + x_noise))
            + y_noise
        )
    ...
    def train_dataloader(self):
        return DataLoader(
            NumpyTensorDataset(self.X, self.Y),
            shuffle=True,
            batch_size=16,
    ...
    def val_dataloader(self):
        return DataLoader(
            TensorDataset(self.X[:100], self.Y[:100]),
            shuffle=True,
            batch_size=32,
            collate_fn=numpy_collate,
        )
...
class BayesLinear(eqx.Module):
    ...
    def __init__(self, in_size, out_size, key):
        ...
        self.weight_mu = (
            jax.random.normal(wkey, (out_size, in_size)) / (out_size + in_size) ** 0.5
        )
    ...
    def kl_div(self):
        ...
        kl_div = jnp.log(1.0) - jnp.log(weight_scale)
...
class BNN(eqx.Module):
    ...
    def __init__(self, key):
        hidden = 51
        self.bnn = eqx.nn.Sequential(
            [
                BayesLinear(1, hidden, key),
...
class JaxLightning(pl.LightningModule):
    ...
    def training_step(self, batch):
        ...
        self.global_step_ += 1
        data, target = (
            jnp.array(batch[0].reshape(-1, *batch[0].shape[1:])),
            jnp.array(batch[1]),
        )
        data = einops.repeat(data, "... -> b ...", b=self.MC)
        target = einops.repeat(target, "... -> b ...", b=self.MC)

        self.key, *subkeys = jax.random.split(
    ...
    def viz_network(self, title=""):
        ...
        for pred in mc_pred.squeeze():
            plt.plot(
                viz_input, pred, color="red", alpha=1 / min(mc_pred.shape[0], 50.0)
            )
        X, Y = self.trainer.datamodule.X, self.trainer.datamodule.Y
        plt.scatter(X, Y, alpha=1 / min(mc_pred.shape[0], 1.0), s=5)
        plt.ylim(-3, 3)
        plt.title(title)
        plt.show()
